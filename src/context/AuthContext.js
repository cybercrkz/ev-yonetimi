...
export const AuthProvider = ({ children }) => {
  ...
  const signOut = useCallback(async () => {
    try {
      const { data: { session: currentSession } } = await browserAuth.getSession();
      if (!currentSession) {
        clearAuthData();
        navigate('/login');
        return;
      ...
      const { error } = await browserAuth.signOut();
      if (error) {
  ...
  const signUp = async (email, password) => {
    try {
      const { data, error } = await browserAuth.signUp({ email, password });
      if (error) throw error;
      toast.success('Kayıt başarılı! (Tarayıcıda saklandı)');
      return data;
    } catch (error) {
  ...
  const signIn = async (email, password) => {
    try {
      const { data, error } = await browserAuth.signInWithPassword({ email, password });
      if (error) throw error;
      setSession(data.session);
      setUser(data.user);
      toast.success('Başarıyla giriş yapıldı (tarayıcı oturumu)');
      return data;
    } catch (error) {
  ...
  const forgotPassword = async (email) => {
    try {
      ...
      if (error) throw error;
      toast.success('Şifre sıfırlama (mock) başarılı — e-posta gönderilmedi');
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        const { data: { session: currentSession }, error } = await browserAuth.getSession();
        if (error) throw error;
        ...
        const { data: { subscription } } = browserAuth.onAuthStateChange(async (event, newSession) => {
          if (mounted) {
