...
const ProfileClean = () => {
  ...
  useEffect(() => {
    ...
    const getProfile = async () => {
      ...
      try {
        ...
        const { data, error } = await browserData.getProfileById(user.id);
        ...
        console.error('Profil bilgileri alınırken hata:', err.message || err);
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error } = await browserData.upsertProfile({
        id: user.id,
      ...
      if (error) {
        setMessage({ text: 'Profil güncellenirken bir hata oluştu: ' + (error.message || ''), type: 'error' });
      } else {
        setMessage({ text: 'Profil bilgileriniz başarıyla güncellendi!', type: 'success' });
      }
    } catch (err) {
      setMessage({ text: 'Profil güncellenirken bir hata oluştu: ' + (err.message || err), type: 'error' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="col-md-8">
          <div className="card shadow">
            <div className="card-body">
              ...
              {message.text && (
                <div className={`alert alert-${message.type === 'success' ? 'success' : 'danger'} mb-4`}>{message.text}</div>
              )}
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  ...
                  <input type="text" className="form-control" id="fullName" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="Ad Soyad" />
                </div>
                <div className="mb-3">
                  <label htmlFor="phone" className="form-label">Telefon</label>
                  <input type="tel" className="form-control" id="phone" name="phone" value={formData.phone} onChange={handleChange} placeholder="05XX XXX XX XX" />
                </div>
                <div className="mb-3">
                  <label htmlFor="address" className="form-label">Adres</label>
                  <textarea className="form-control" id="address" name="address" value={formData.address} onChange={handleChange} rows="3" placeholder="Adres bilgileriniz"></textarea>
                </div>
